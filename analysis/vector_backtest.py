...
def momentum_signal(close: pd.Series, lookback: int = 60) -> pd.Series:
    return np.sign(signed_price_return(close, lookback)).fillna(0.0)
...
def vol_scaled_momentum_signal(
    ...
    vol = signed_price_return(close).rolling(vol_window).std() * np.sqrt(252)
...
def run_vector_backtest(
    ...
    out["asset_return"] = signed_price_return(out["close"]).fillna(0.0)

    # Signal from day t is applied to day t+1 price change.
